...
const ManageCategoriesModal = ({ isOpen, onClose, expenseCategories, incomeCategories, onDelete }) => {
  ...
  const CategoryList = ({ title, categories }) => (
    <div className="mb-6">
      <h3 className="text-xl font-semibold mb-2 text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700 pb-1">{title}</h3>
      {categories.length > 0 ? (
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm italic">No custom categories defined for this type.</p>
      )}
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-lg">
        <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-2">Manage Custom Categories</h2>
        ...
        {hasCustomCategories ? (
          ...
          <p className="text-gray-500 dark:text-gray-400 text-center py-8">You haven't added any custom categories yet.</p>
        )}

        <div className="flex justify-end mt-6">
          <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-300 dark:bg-gray-600 dark:text-white rounded hover:bg-gray-400 dark:hover:bg-gray-500">Close</button>
        </div>
      </div>
    </div>
  );
